...
const getdata = () => {
    ...
    draw(x, y);
    Spline_Interpolation(x, y, a);

}

const Spline_Interpolation = (x, y, a) => {
    var n = x.length;
    var table = document.getElementById("output");
    var table2 = document.getElementById("outputR");
    var error = 0;
    if (document.getElementById("outputR").getElementsByTagName("tr").length > 0) {
        cleantable();
    }
    //console.log(RENewton_Interpolation(x,y,2,0));

    result = spline(a[0], x, y);

    error = Math.abs(result - a[1]);
