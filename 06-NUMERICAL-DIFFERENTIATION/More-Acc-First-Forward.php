<h1>More Acc First Forward</h1>

<body onload="More_Acc_First_Forward(); draw();">
    <div class="content">
            ...
            <div class="card">
                ...
                <div class="card-footer">
                    <button type="button" class="btn btn-primary btn-lg btn-block"
                        onclick="More_Acc_First_Forward(); draw(); ">ENTER</button>
                </div>
            </div>
            <br>
            <div class="card">
...
const More_Acc_First_Forward = () => {
    ...
    if (n == 1) {
        result = (-funcal(x + 2 * h, expression) + 4 * funcal(x + h, expression) - 3 * funcal(x, expression));
        result = result / (2 * h)
    } else if (n == 2) {
        result = (-funcal(x + 3 * h, expression) + 4 * funcal(x + 2 * h, expression) - 5 * funcal(x + h,
            expression) + 2 * funcal(x, expression));
        result = result / math.pow(h, 2);
    } else if (n == 3) {
        result = (-3 * funcal(x + 4 * h, expression) + 14 * funcal(x + 3 * h, expression) - 24 * funcal(x + 2 * h,
            expression) + 18 * funcal(x + h, expression) - 5 * funcal(x, expression));
        result = result / (2 * math.pow(h, 3));
    } else {
        result = (-2 * funcal(x + 5 * h, expression) + 11 * funcal(x + 4 * h, expression) - 24 * funcal(x + 3 * h,
            expression) + 26 * funcal(x + 2 * h, expression) - 14 * funcal(x + h, expression) + 3 * funcal(
            x, expression));
        result = result / (math.pow(h, 4));
    }
